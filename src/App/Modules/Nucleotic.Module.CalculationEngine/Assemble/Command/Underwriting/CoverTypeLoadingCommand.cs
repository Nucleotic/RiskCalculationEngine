...
{
    public class CoverTypeLoadingCommand : UnderwritingCalculatorCommandBase
    {
        /// <summary>
        ...
        public override void Assemble(LoadingsContext assembly)
        {
            try
            {
                ...
                try
                {
                    ...
                    var function = FunctionSet.CoverTypeLoadingCalculation;
                    assembly.Loadings.Add(new AssetLoading {LoadingName = nameof(CalculatorFunctionType.CoverType), LoadingValue = calculator.CalculateCoverTypeLoading(function), Order = assembly.Loadings.Count + 1});
                }
                catch (Exception ex)
                {
