...
{
    public class AgeBandLoadingCalculationCommand : UnderwritingCalculatorCommandBase
    {
        /// <summary>
        ...
        public override void Assemble(LoadingsContext assembly)
        {
            try
            {
                ...
                try
                {
                    ...
                    var function = FunctionSet.AgeLoadingCalculation;
                    assembly.Loadings.Add(new AssetLoading {LoadingName = nameof(CalculatorFunctionType.Age), LoadingValue = calculator.CalculateAgeLoading(function), Order = assembly.Loadings.Count + 1});
                }
                catch (Exception ex)
                {
