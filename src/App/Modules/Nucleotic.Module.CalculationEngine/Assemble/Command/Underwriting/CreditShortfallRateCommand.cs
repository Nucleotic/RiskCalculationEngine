...
{
    public class CreditShortfallRateCommand : CreditCalculatorCommandBase
    {
        /// <summary>
        ...
        public override void Assemble(LoadingsContext assembly)
        {
            try
            {
                ...
                try
                {
                    ...
                    var function = FunctionSet.CreditShortFallRateCalculation;
                    assembly.Loadings.Add(new AssetLoading
                    {
                        LoadingName = nameof(CalculatorFunctionType.CreditShortfall),
                        LoadingValue = calculator.CalculateCreditShortFallRate(function),
                        DoNotAggregateValue = true,
                        Order = assembly.Loadings.Count + 1
                    });
                }
                catch (Exception ex)
                {
